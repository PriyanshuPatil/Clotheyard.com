...
const Facebook = ({boxSize,ml,mt,color}) =>{
    return(
        <Icon as={TfiFacebook} boxSize={boxSize} ml={ml} mt={mt} color={color} />
    )
}

const Google = ({boxSize,ml,mt,color}) =>{
    return(
        <Icon as={ImGoogle} boxSize={boxSize} ml={ml} mt={mt} color={color}/>
    )
}

const  Linkedin = ({boxSize,ml,mt,color}) =>{
    return(
        <Icon as={TfiLinkedin} boxSize={boxSize} ml={ml} mt={mt} color={color}/>
    )
}
...
{
 return(
    <Grid templateColumns='repeat(3, 1fr)' gap={2} margin='auto' width={['55%','28%','40%']} mt={7}>
    <GridItem 
